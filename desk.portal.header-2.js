...
function deskEV(v) {
    ...
    if ($('#q').length != 0) {
	    ...
	    $("#a-content-select").change(function(event){
	      ...
	      if(r && r.length>1)
	        location.href = "http://assets2.desk.com/customer/" + $(this).val() + "/portal" + r[2] + location.search;
	      else
	        location.href = "http://assets2.desk.com/customer/" + $(this).val() + "/portal/articles" + location.search;
	    });
	});
...
jQuery(document).ready(function() {
	...
	// For the back button links (previously <a href="http://assets2.desk.com/" onclick="history.back(); return false;"> )
    ...
    $("a[href='http://assets2.desk.com/']").attr("href", home_link );
});
